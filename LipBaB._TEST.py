...
weights.append(w2.tolist())
biases.append(b1.tolist())
biases.append(b2.tolist())
weights.append(w3.tolist())
biases.append(b3.tolist())

act_pat=[]
for e in ls:
    act_pat.append([[1,1]for i in range(e)])

bnds=[[0.,3.14] for i in range(ls[0])]
...
while(True):
    ...
    #if (time.time() - start_time)>=1000: #(setting execution time limit (in seconds))
        #break
    if(pq.empty()==True):
        break
    tp=pq.get()[2]
    ...
    i +=1
    if i%100==0:
        print(int(time.time() - start_time),i, tp.Lub)
    branch(tp,-1)
    branch(tp,1)
